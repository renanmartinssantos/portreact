            ...
            <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-5 mt-5">
                ...
                <div className="p-5">
                    <a href={repo.link} target="_blank">
                        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{repo.title}</h5>
                    </a>
                    <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{props.isEnglish ? repo.descriptionUS : repo.descriptionBR }</p>
                    <a href={repo.link} target="_blank" className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-sky-950 rounded-lg hover:bg-sky-900 focus:ring-4 focus:outline-none focus:ring-blue-300">
